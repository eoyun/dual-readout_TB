...
int main(int argc, char** argv) {
    ...
    float psPIDcut = 13630 * 2.5f-18000; // This is not the proper way to set cut!! Details will be presented in workshop
    ...
    */
    ...
    TFile * root_fp = new TFile(Form("FWHM_%d_SiPM_PID.root",runNum),"recreate");
    for (int i=0;i<200;i++){
        FWHM_S[i]->Write();
        FWHM_C[i]->Write();
    }

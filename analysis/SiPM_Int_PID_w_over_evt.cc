...
int main(int argc, char** argv) {
    ...
    float psPIDcut = 13630 * 5.f-18000; // This is not the proper way to set cut!! Details will be presented in workshop
    ...
    */
    ...
    TH1F *FWHM_S[200];
    TH1F *FWHM_C[200];
    TH1F *IntADC_single_S[200];
    TH1F *IntADC_single_C[200];
    for (int i=0;i<200;i++){
	FWHM_S[i] = new TH1F(Form("FWHM_%d_column_%d_plate_S",(i/10+1)%2+i%10*2+21,i/10+21),";bin;evt",1000,0,1000);
	FWHM_C[i] = new TH1F(Form("FWHM_%d_column_%d_plate_C",(i/10)%2+i%10*2+21,i/10+21),";bin;evt",1000,0,1000);
	IntADC_single_S[i] = new TH1F(Form("IntADC_%d_column_%d_plate_S",(i/10+1)%2+i%10*2+21,i/10+21),";bin;evt",1000,-100,5000);
	IntADC_single_C[i] = new TH1F(Form("IntADC_%d_column_%d_plate_C",(i/10)%2+i%10*2+21,i/10+21),";bin;evt",1000,-100,5000);
	...
	for (int i=0;i<200;i++){
	    ...
	    short max_S = *max_element(SiPM_s_waveform.begin(),SiPM_s_waveform.end()-23);
	    short max_C = *max_element(SiPM_c_waveform.begin(),SiPM_c_waveform.end()-23);
	    ...
	    short maxindex_S = max_element(SiPM_s_waveform.begin(),SiPM_s_waveform.end()-23)-SiPM_s_waveform.begin();
	    short maxindex_C = max_element(SiPM_c_waveform.begin(),SiPM_c_waveform.end()-23)-SiPM_c_waveform.begin();
	    int first_s=0;
	    int first_c=0;
	    int last_s=0;
	    int last_c=0;
	    ...
	    double ped_s =0.;
	    double ped_c =0.;
	    for (bin=1;bin<101;bin++){
		ped_s+=SiPM_s_waveform.at(bin)/100.;
		ped_c+=SiPM_c_waveform.at(bin)/100.;
	    }
	    short cut_s = (short)(ped_s-min_S)*0.5;
	    short cut_c = (short)(ped_c-min_C)*0.5;
	    if (max_S-ped_s>50){
	        if (maxindex_S>100 && maxindex_S<250){
	            if (SiPM_s_waveform.at(maxindex_S-1)-ped_s>50 &&SiPM_s_waveform.at(maxindex_S+1)-ped_s>50) flag_over_s = 1;
                }
	    }
	    if (max_C-ped_c>50){
	        if (maxindex_C>100 && maxindex_C<250){
	            if (SiPM_c_waveform.at(maxindex_C-1)-ped_c>50 &&SiPM_c_waveform.at(maxindex_C+1)-ped_c>50) flag_over_c = 1;
                }
	    }
	    for (bin =0; bin < 150; bin++){
		if (minindex_S>100 && minindex_S<250){
		    if (cut_s>ped_s-SiPM_s_waveform[minindex_S-bin]){
		...
		if (minindex_C>100 && minindex_C<250){
		    if (cut_c>ped_c-SiPM_c_waveform[minindex_C-bin]){
	   ...
	   if (minindex_S>100 && minindex_S<250 && last_s-first_s>2 && flag_over_s == 0){
		FWHM_S[i]->Fill(last_s-first_s);
		for (bin=100;bin<250;bin++){
		    sum_S += ped_s-SiPM_s_waveform[bin+1];
		    sum_single_S += ped_s-SiPM_s_waveform[bin+1];
		}
	   }
	   if (minindex_C>100 && minindex_C<250 && last_c-first_c>2 && flag_over_c == 0){
		FWHM_C[i]->Fill(last_c-first_c);
		for (bin=100;bin<250;bin++){
		    sum_C += ped_c-SiPM_c_waveform[bin+1];
		    sum_single_C += ped_c-SiPM_c_waveform[bin+1];
		}
	   }
	   //FWHM_S[i]->Fill(last_s-first_s);
	   //FWHM_C[i]->Fill(last_c-first_c);
	   ...
	   flag_c_last=0;
           ...
           last_s =0;
           last_c =0;
           if ( dwcCorrelationPID(dwc1_correctedPosition, dwc2_correctedPosition, 1.5f) && psIntADC > psPIDcut ) {
	      IntADC_single_S[i]->Fill(sum_single_S);
 	      IntADC_single_C[i]->Fill(sum_single_C);
	   }
	}
    ...
    TFile * root_fp = new TFile(Form("FWHM_%d_SiPM_PID_w_250_5mip_over_evtped.root",runNum),"recreate");

    for (int i=0;i<200;i++){
        FWHM_S[i]->Write();
        FWHM_C[i]->Write();
        IntADC_single_S[i]->Write();
        IntADC_single_C[i]->Write();
    }
    IntADC_S->Write();
    IntADC_C->Write();
    IntADC_SvsC->Write();

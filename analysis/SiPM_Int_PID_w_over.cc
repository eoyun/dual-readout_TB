...
int main(int argc, char** argv) {
    ...
    */
	...
	for (int i=0;i<200;i++){
	    ...
	    if (max_S-S_ped[i]>50){
	        if (maxindex_S>100 && maxindex_S<250){
	            if (SiPM_s_waveform.at(maxindex_S-1)-S_ped[i]>50 &&SiPM_s_waveform.at(maxindex_S+1)-S_ped[i]>50) flag_over_s = 1;
                }
	    }
	    if (max_C-C_ped[i]>50){
	        if (maxindex_C>100 && maxindex_C<250){
	            if (SiPM_c_waveform.at(maxindex_C-1)-C_ped[i]>50 &&SiPM_c_waveform.at(maxindex_C+1)-C_ped[i]>50) flag_over_c = 1;
                }
	    }
	   ...
	   if (minindex_S>100 && minindex_S<250 && last_s-first_s>2 && flag_over_s == 0){
		...
		for (bin=100;bin<250;bin++){
		    ...
		    sum_single_S += S_ped[i]-SiPM_s_waveform[bin+1];
		}
	   }
	   if (minindex_C>100 && minindex_C<250 && last_c-first_c>2 && flag_over_c == 0){
		...
		for (bin=100;bin<250;bin++){
		    ...
		    sum_single_C += C_ped[i]-SiPM_c_waveform[bin+1];
		}
	   }
	   //FWHM_S[i]->Fill(last_s-first_s);
	   //FWHM_C[i]->Fill(last_c-first_c);
    ...
    TFile * root_fp = new TFile(Form("FWHM_%d_SiPM_PID_w_250_5mip_over.root",runNum),"recreate");

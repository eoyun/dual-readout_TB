...
int main(int argc, char** argv) {
    ...
    float psPIDcut = 13630 * 2.5f; // This is not the proper way to set cut!! Details will be presented in workshop
    ...
    */
    ...
    TH2F *IntADC_SvsC = new TH2F("peakADC_SvsC",";intADC(S);intADC(C)",1000,-1000,20000,1000,-1000,10000);
    ...
    TH2F *IntADC_SvsC_PID = new TH2F("peakADC_SvsC_PID",";intADC(S);intADC(C)",1000,-1000,20000,1000,-1000,10000);
    TH1F *FWHM_S[200];
    TH1F *FWHM_C[200];
    for (int i=0;i<200;i++){
    ...
    TFile * root_fp = new TFile(Form("Peak_%d_SiPM_PID.root",runNum),"recreate");
    for (int i=0;i<200;i++){
        FWHM_S[i]->Write();
        FWHM_C[i]->Write();
    }
